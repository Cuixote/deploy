...
const { recordTime, silentLog, execCommand } = require('../utils/common')
...
module.exports = function upload () {
  ...
  // 输出结果中有失败数
  if (+fail > 0) {
    throw new Error('上传失败！更多信息请在log文件中查看: cat ' + logPath)
  } else {
    log('发布结果如下：')
  ...
  // 发送index.html到服务器
  if (!config.server) {
    throw new Error('请设置服务器列表')
  }
  const serverList = config.server[env] || []
